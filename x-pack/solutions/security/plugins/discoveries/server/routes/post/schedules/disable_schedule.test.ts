...
jest.mock('../../helpers/is_attack_discovery_workflows_enabled', () => ({
  isAttackDiscoveryWorkflowsEnabled: jest.fn().mockResolvedValue(true),
}));

import { isAttackDiscoveryWorkflowsEnabled } from '../../helpers/is_attack_discovery_workflows_enabled';
const mockIsAttackDiscoveryWorkflowsEnabled =
  isAttackDiscoveryWorkflowsEnabled as jest.MockedFunction<
    typeof isAttackDiscoveryWorkflowsEnabled
  >;
...
describe('registerDisableScheduleRoute', () => {
  ...
  it('returns 200 with the disabled schedule id on success', async () => {
    ...
    const handler = addVersionMock.mock.calls[0][1];
    mockDisableSchedule.mockResolvedValue(undefined);
    ...
    expect(mockDisableSchedule).toHaveBeenCalledWith({ id: 's1' });
  ...
  it('returns a custom error when the disable fails', async () => {
    ...
    const handler = addVersionMock.mock.calls[0][1];
    mockDisableSchedule.mockRejectedValue(new Error('disable failed'));
    ...
    expect(logger.error).toHaveBeenCalledWith('Error disabling schedule s1: disable failed');
  });
});
