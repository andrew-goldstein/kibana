...
import { SCHEDULES_ENABLE_PATH } from '../../../lib/schedules/constants';
...
export const registerEnableScheduleRoute = (
  router: IRouter,
  logger: Logger,
  {
    analytics,
    ...
    .addVersion(
      {
        ...
        validate: {
          request: {
            params: EnableAttackDiscoveryScheduleRequestParams,
          },
        },
      },
      async (context, request, response) => {
        ...
        try {
          ...
          logger.error(
            `Error enabling schedule ${id}: ${err instanceof Error ? err.message : err}`
          );
